...
def buscar_cep(cep_input):
    # Limpa o CEP: remove qualquer coisa que não seja número para a busca
...
with col1:
    cpf = st.text_input("4. CPF *", key="cpf_key", on_change=aplicar_mascaras, max_chars=14, placeholder="000.000.000-00")
    email_paciente = st.text_input("E-mail *")
with col2:
    telefone = st.text_input("5. Telefone (WhatsApp) *", key="tel_key", on_change=aplicar_mascaras, max_chars=15, placeholder="(00) 00000-0000")
    nascimento = st.text_input("6. Data de Nascimento *", key="nasc_key", on_change=aplicar_mascaras, max_chars=10, placeholder="00/00/0000")

st.divider()

# --- ENDEREÇO ---
st.subheader("Endereço")
cep_input = st.text_input("7. CEP *", max_chars=10, placeholder="00000-000")

# Lógica de Endereço Automático
logradouro_auto, bairro_auto, cidade_auto, uf_auto = "", "", "", ""
if cep_input:
...
rua = st.text_input("Logradouro (Rua/Avenida) *", value=logradouro_auto)
complemento = st.text_input("8. Número e Complemento *")

col3, col4 = st.columns(2)
with col3:
    bairro_field = st.text_input("Bairro *", value=bairro_auto)
    cidade_field = st.text_input("Cidade *", value=cidade_auto)
with col4:
    uf_field = st.text_input("Estado (UF) *", value=uf_auto)
...
encaminhamento = st.text_input("11. Possui encaminhamento? (Informe o solicitante) *")
demanda = st.text_area("12. Descreva sua demanda (Motivo da avaliação) *", height=100)

if st.button("Enviar Formulário"):
    # Lista detalhada de todos os campos para validação rigorosa
    lista_validacao = {
        "Nome": nome, "CPF": cpf, "Email": email_paciente, "Telefone": telefone, 
        "Nascimento": nascimento, "CEP": cep_input, "Rua": rua, 
        "Complemento": complemento, "Bairro": bairro_field, "Cidade": cidade_field, 
        "UF": uf_field, "Escolaridade": escolaridade, "Profissão": profissao, 
        "Encaminhamento": encaminhamento, "Demanda": demanda
    }
    
    if any(not str(v).strip() for v in lista_validacao.values()):
        st.error("Por favor, preencha todos os campos obrigatórios (*). Nenhum campo de endereço pode ficar em branco.")
    else:
        dados_finais = {
            ...
            "CEP": cep_input,
            "Endereço Completo": f"{rua}, {complemento} - {bairro_field}. {cidade_field}/{uf_field}",
